...
class DashboardController extends Controller
{
    ...
    public function index(){
		...
		if($listed->count() > 0){
			...
			$data['pending_reservations']= Reservation::with('users','rooms')->where('type','!=','contact')
			                                           ->where(function($query){
				                                           $query->where('status','Pending')->orwhere('status','Pre-Accepted');
			                                           })
			                                           ->where('checkin', '>=', date('Y-m-d'))
			...
			$messages = Chats::where('user_id', Auth::user()->id)->where("is_read", 0)->groupBy('sender_id')->get();
			$data['unreadmessagecount'] = Chats::where('user_id', Auth::user()->id)->where('sender_id', '!=', Auth::user()->id)->where("is_read", 0)->count();
			$data['messages'] = [];
			foreach($messages as $message){
				$last_message = Chats::where('user_id', Auth::user()->id)->where("sender_id", $message->sender_id)
								->where('type', 'text')->orderBy('created_at', 'desc')->first();
				$messageArray['message'] = $last_message;
				// $messageArray['message_time'] = $last_message->created_at;
				$messageArray['count'] = Chats::where('user_id', Auth::user()->id)->where("sender_id", $message->sender_id)
										->where('type', 'text')->where("is_read", 0)->orderBy('created_at', 'desc')->count();
				$messageArray['userInfo'] = User::find($message->sender_id);
	...
	public function getapikeys(){
		...
		return array(
			'public_api_key' =>	$user->public_api_key,
			'secret_api_key' => $user->secret_api_key
		);
	}
	public function generateapikeys(){
		$public_key = (rand(1000, 9999).Auth::user()->id);
		$secret_key = implode('-', str_split(substr(strtolower(md5(microtime().rand(1000, 9999).Auth::user()->id)), 0, 30), 6));
		$user = User::find(Auth::user()->id);
		$user->public_api_key = $public_key;
		$user->secret_api_key = $secret_key;
		$user->save();
		return array(
