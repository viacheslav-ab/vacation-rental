...
class Calendar extends React.Component {
    constructor(props) {
        ...
        this.handleSettingDailyPrice = this.handleSettingDailyPrice.bind(this)
        ...
        this.handleEditDailyPrice = this.handleEditDailyPrice.bind(this)
        this.handleRemoveDailyPrice = this.handleRemoveDailyPrice.bind(this)
    }
    componentDidMount() {
    ...
    bookingCalendarInit() {
        ...
        // axios.post('/ajax/rooms/manage-listing/' +  this.props.match.params.roomId + '/' + dateFns.getYear(this.state.currentMonth) + '/' + (dateFns.getMonth(this.state.currentMonth) + 1) + '/get_daily_prices')
        //     .then(result => {
        //         this.setState({
        //             daily_calendar: result.data
        //         })
        //     })
    ...
    handleBookingBlocked(e) {
        ...
        if (this.state.edit_blocked) {
            ...
            blocked_data.seasonal_name = blocked_data.seasonal_name=='' || blocked_data.seasonal_name ==undefined || blocked_data.seasonal_name==null? "no name": blocked_data.seasonal_name;
            ...
            .then(result => {
                if (result.data.success == 'true') {
                    console.log('blocked saving success');
                    this.closeReservationModal()
                    this.bookingCalendarInit()
                }
                else {
    ...
    handleBookingReservation(e) {
        ...
        if (this.state.edit_reservation) {
            ...
            reservation_data.seasonal_name = reservation_data.seasonal_name=='' || reservation_data.seasonal_name==undefined || reservation_data.seasonal_name==null ? "no name": reservation_data.seasonal_name;
    ...
    handleBookingSeasonal(e) {
        ...
        if (this.state.edit_season) {
            ...
            seasonal_data.seasonal_name = seasonal_data.seasonal_name=='' || seasonal_data.seasonal_name==undefined || seasonal_data.seasonal_name==null? "no name": seasonal_data.seasonal_name;
    ...
    handleSettingDailyPrice(e) {
        //    console.log(e, '+++++++++++++++++++++')  
        e.preventDefault()
        console.log('Daily Price Saving')

        if (this.state.edit_daily) {
            let daily_data = this.state.month_calendar_data.daily_detail[this.state.edit_daily_index]
            
            axios.post('/ajax/rooms/manage-listing/' + this.props.match.params.roomId + '/save_daily_prices', {
                data: {
                    start_date: dateFns.format(this.state.startDate, dateFormat1),
                    end_date: dateFns.format(this.state.endDate, dateFormat1),
                    reservation_source: 'Calendar',
                    price: this.state.daily_price ? this.state.daily_price : 0
                }
            })
                .then(result => {
                    if (result.data.success == 'true') {
            ...
            axios.post('/ajax/rooms/manage-listing/save_daily_prices', {
                data: {
                    start_date: dateFns.format(this.state.startDate, dateFormat1),
                    end_date: dateFns.format(this.state.endDate, dateFormat1),
                    reservation_source: 'Calendar',
    ...
    handleRemoveBlocked(index, value) {
        ...
        axios.post(`/ajax/rooms/manage-listing/${this.props.match.params.roomId}/delete_not_available_days`, {
    ...
    handleRemoveReservation(index, value) {
        ...
        axios.post(`/ajax/rooms/manage-listing/${this.props.match.params.roomId}/delete_reservation`,{
    ...
    handleEditDailyPrice(index, value) {
        let daily_data = this.state.month_calendar_data.daily_detail[index]
        this.setState({
            ...
            tabIndex: 0,
            edit_daily_id: value,
            edit_daily_index: index
        })
        console.log(value, 'Daily Id')
    }

    handleRemoveDailyPrice(index, value) {
        // alert(index);
        // alert(value);
        //delete_reservation
        let daily_data = this.state.month_calendar_data.daily_detail[index]
        alert(reservation_data.seasonal_name);
        axios.post(`/ajax/rooms/manage-listing/${this.props.match.params.roomId}/delete_daily_price`, { daily_price_id: daily_data.price })
            .then(result => {
                console.log(result.data);
                this.bookingCalendarInit()
            })
    ...
    handleRemove(type, index, value){
        if(type == 1){
            this.handleRemoveSeasonal(index, value);
        }
        else if(type == 2)
        {
            this.handleRemoveReservation(index, value);
        }
        else if(type == 3){
            this.handleRemoveBlocked(index, value);
        }
        else if(type == 4) {
            this.handleRemoveDailyPrice(index, value);
        }
    }
    handleEdit(type, index, value){
        ...
        else if(type == 2)
        {
            ...
            this.handleEditDailyPrice(index, value);
        }
    }
    render() {
        ...
        const seasonal_columns = [
            {
                ...
                Cell: props => <div className="w-100">
                    ...
                    <div className="row col-md-12"><span className="mx-auto">({moment.utc(props.original.start_date).format(dateFormat)   } - {moment.utc(props.original.end_date).format(dateFormat)})</span></div>
                </div>
            }, {
                Header: 'Nightly',
                style: { width: '100%', textAlign: 'center' },
        ...
        const reservation_columns = [
            {
                ...
                Cell: props => <div className="w-100">
                    <div className="row col-md-12">
                        <strong className="mx-auto">{props.value}</strong> </div>
                    <div className="row col-md-12"><span className="mx-auto">({moment.utc(props.original.start_date).format(dateFormat)}-{moment.utc(props.original.end_date).format(dateFormat)} - {moment.utc(props.original.end_date).format(dateFormat)})</span></div>
                </div>
            },
            {
                Header: 'Price',
        ...
        const daily_columns = [
            ...
            },
            {
                ...
                style: { width: '100%', textAlign: 'center' },
                accessor: 'price',
                Cell: props => <span className='text-center'>{month_calendar_data && month_calendar_data.daily_price ? <span dangerouslySetInnerHTML={{ __html: month_calendar_data.daily_price.currency.symbol }}></span> : '$'}<strong>{props.value}</strong></span> // Custom cell components!
            },
            {
                Header: 'Guests',
                style: { width: '100%', textAlign: 'center' },
           ...
           
            {
                ...
                accessor: 'id',
                Cell: props => <div>
                    <a href="javascript:;" className="table_edit" onClick={() => this.handleEditDailyPrice(props.index, props.value)}><i className="fa fa-edit"></i></a>
                    <a href="javascript:;" className="delete_details delete_seasonal" onClick={() => this.handleRemoveDailyPrice(props.index, props.value)}><i className="fa fa-trash"></i></a>
                </div>
            }
        ...
        return (
            <div className="manage-listing-content-wrapper clearfix">
                ...
                >
                    ...
                    <Tabs selectedIndex={this.state.tabIndex} onSelect={tabIndex => this.setState({ tabIndex })}>
                        <TabList className="tabs tabba form-tab-header">
                            ...
                            {/* <Tab className="tab-item text-lead h5 mylists">Daily Prices</Tab> */}
                        ...
                        {/* <TabPanel>
                            <form id="unavailable_form_t" data-mode="create" className=" " noValidate="novalidate" onSubmit={this.handleSettingDailyPrice} >
                                <div className="seasonal_price">
                                    <div className="ses_time datepicker-wrapper">
                                        <input type="hidden" name="room_id" defaultValue={11475} className="tooltipstered" />
                                        <div className="col-md-6 col-sm-6 col-6 ses_pop row">
                                    ...
                                    <div className="ses_time">
                                        <div className="col-md-12 col-sm-12 col-12 ses_pop1">
                                            ...
                                            <div className="pricelist">
                                                <div className="price_doller" dangerouslySetInnerHTML={{ __html : this.state.month_calendar_data.rooms_price ? this.state.month_calendar_data.rooms_price.currency.original_symbol : '$' }}></div>
                                                <input type="text" id="daily_price_t" name="daily_price" value={this.state.daily_price} onChange={this.handleChangeModal} className="  tooltipstered valid " aria-invalid="false" />
                                            </div>
                                            <p data-error="price" className="ml-error" />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="col-md-6 col-sm-6 col-6 text-right">
                                            <button className="day_save" type="submit" id="save_daily_t" name="save">Save</button>
                                            <a className="day_delete d-none delete_not_available mt-2" id="delete_daily_t">Delete</a>
                                        </div>
                                    </div>
                                    <div className="loading global-ajax-form-loader" style={{ visibility: 'hidden' }} />
                                </div>
                ...
                <div className="col-md-3 import_calander border-left">
                    ...
                    <div className="row">
                        <div className="col-sm-12 p-header-line">
                            <p className="p-title">Base Price</p>
                        </div>
                        <div className="row col-sm-12">    
                            <p className="item-title col-sm-6" className="col-sm-6">Nightly</p>
                            <span className="item-content col-sm-6">
                                {month_calendar_data && month_calendar_data.rooms_price && month_calendar_data.rooms_price.currency ? <span dangerouslySetInnerHTML={{ __html: month_calendar_data.rooms_price.currency.symbol }}></span> : <span>$</span>}
                        ...
                        <div className="row col-sm-12">    
                            <p className="item-title col-sm-6" className="col-sm-6">Weekly</p>
                            <span className="item-content col-sm-6">
                                {month_calendar_data && month_calendar_data.rooms_price && month_calendar_data.rooms_price.currency ? <span dangerouslySetInnerHTML={{ __html: month_calendar_data.rooms_price.currency.symbol }}></span> : <span>$</span>}
                        ...
                        <div className="row col-sm-12">    
                            <p className="item-title col-sm-6" className="col-sm-6">Monthly</p>
                            <span className="item-content col-sm-6">
                                {month_calendar_data && month_calendar_data.rooms_price && month_calendar_data.rooms_price.currency ? <span dangerouslySetInnerHTML={{ __html: month_calendar_data.rooms_price.currency.symbol }}></span> : <span>$</span>}
                        ...
                        <div className="row col-sm-12">    
                            <p className="item-title col-sm-6" className="col-sm-6">Min.Stay</p>
                            <span className="item-content col-sm-6">
                    ...
                    {/* <div className="row">
                        <div className="p-header-line col-sm-12">
                            <p className="p-title">Daily Prices </p>
                        </div>
                        <div className="col-sm-12 p-0">
                            <RatePanel
                                data={month_calendar_data && month_calendar_data.daily_dates ? month_calendar_data.daily_dates : []}
