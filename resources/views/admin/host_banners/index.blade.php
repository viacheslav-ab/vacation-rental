                                        ...
                                        <table id="basic-btn" class="table table-striped table-bordered nowrap">
                                            ...
                                            <tbody>
                                                @if(count($data) != 0)
                                                    ...
                                                    <tr>
                                                        <td>{{$row->id}}</td>                            
                                                        <td><img src="{{asset("images/host_banners/$row->image")}}" alt="Host Banner Image" style="height:100px"></td>
                                                        <td>{{$row->title}}</td>
                                                        <td style="max-width:500px !important;white-space: inherit;">{{$row->description}}</td>                            
                                                        <td>{{$row->link_title}}</td>                            
                                                        <td>{{$row->link}}</td>                            
                                                        <td class="text-center">
                                                            <a href="{{url( 'admin/edit_host_banners/' . $row->id )}}" class="btn btn-xs btn-primary" data-toggle="tooltip" title="" data-original-title="Edit" aria-label="Edit host banner">
                                                                <i class="icofont icofont-edit"></i>
                                                            </a>
                                                            &nbsp;
                                                            <button data-id="{{ $row->id }}" class="btn btn-xs btn-danger delete-btn" aria-label="Delete host banner">
