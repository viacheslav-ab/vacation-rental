...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return $data;
        $data['reservation'] = Reservation::where('list_type', 'Rooms')->whereHas('host_users', function ($query) {
            $query->where('status', 'Active');
        })->orderBy('id', 'desc')->where('status', 'Accepted')->groupBy('room_id')->limit(10)->get();
        $data['view_count'] = Rooms::whereHas('users', function ($query) {
            $query->where('status', 'Active');
        })->orderBy('views_count', 'desc')->where('status', 'Listed')->groupBy('id')->get();
        $data['recommented'] = Rooms::whereHas('users', function ($query) {
            $query->where('status', 'Active');
        })->orderBy('id', 'desc')->where('recommended', 'Yes')->where('status', 'Listed')->groupBy('id')->get();
    ...
    {
        $pages = Page::where('footer', 'yes')->where('status', 'Active')->get();
        foreach ($pages as $key => $page) {
            $pages[$key]['permallink'] = $page->permalink;
        }
        return $pages;
     }
}
