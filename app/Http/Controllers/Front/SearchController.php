...
class SearchController extends Controller
{
	...
	public function index(Request $request){
        ...
        // return $this->helper->custom_strtotime($request->input('checkout'), $php_date_format);
    ...
    public function searchResult(Request $request){
        ...
        if($request->searchOption == 'address'){
            if($full_address){
                $address      = str_replace([" ","%2C"], ["+",","], "$full_address");
                $geocode      = @file_get_contents('https://maps.google.com/maps/api/geocode/json?key=AIzaSyA34nBk3rPJKXaNQaSX4YiLFoabX3DhkXs&address='.$address.'&sensor=false&libraries=places');
                $json         = json_decode($geocode);
            }
            else{
                $user_ip = $_SERVER['REMOTE_ADDR'];
