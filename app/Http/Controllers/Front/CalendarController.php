...
use App\Models\Front\DailyPrice;
use Validator;
use Form;
...
class CalendarController extends Controller
{
	...
	public function get_calendar_data($room_id, $year = '', $month = '')
    {
        ...
        while($day <= $total_dates)
        {
            ...
            $prev_date = date('Y-m-d', strtotime('-1 day', strtotime($this_date)));
            $tmp = Calendar::where('room_id', $room_id)->where('date', $this_date)->first()['seasonal_name'];

            $calendar_data[$k]['seasonal_name'] = $tmp;
            $calendar_data[$k]['price_display'] = ' style="display: none;"';
            $prev_season = Calendar::where('room_id', $room_id)->where('date', $prev_date)->first()['seasonal_name'];
            if($prev_season == null) $prev_season = '';
            $calendar_data[$k]['prev_price'] = $rooms_price->price($prev_date);
            $calendar_data[$k]['prev_seasonal'] = $prev_season;
        ...
        $data['reservation_detail'] = Reservation::where('room_id',$room_id)->where(function($query) {
            $query->where([['status', '<>', 'Cancelled'], ['ba_status', 'Confirmed']]);  
          })->get()->toArray();
        //   var_dump($data['reservation_detail']);
        $scalars = array_map(function ($item) {
            $start_time = strtotime($item['checkin']);
            $last_time = strtotime($item['checkout']);
        ...
        $data['reservation_detail'] = $scalars;

        $not_available_dates = "";
        $not_available_dates = Reservation::where('room_id',$room_id)->where(function($query) {
            $query->where([['status', '<>', 'Cancelled'], ['ba_status', 'Black']]);  
          })->get()->toArray();
        
        $not_available_dates = array_map(function ($item) {
            $start_time = strtotime($item['checkin']);
            $last_time = strtotime($item['checkout']);
            $diff = abs($last_time - $start_time);
        ...
        $data['blocked_detail'] = $not_available_dates;
    ...
    /* get_unavailable_calendar  */
    public function get_unavailable_calendar(Request $request){
        $id  = $request->id;  
	...
	public function unavailable_calendar(Request $request)
    {
        ...
        foreach ($unavailable_dates as $day) {            
             ...
             * the above condition is Guru's
             * can't synchronize with this condition because we have to sync
             */
	 ...
	 * @return daily prices
	 */
	public function get_daily_prices($room_id, $year = '', $month = '')
    {
        $result = DailyPrice::where('room_id', $room_id)->orderBy('start_date', 'asc')->get();
        return $result;
    }

    public function save_daily_prices(Request $request) {
        $data = $request->data;
        ...
        if(isset($data['id']) && $data['id']!='') {
            $season['id'] = $data['id'];
            DailyPrice::updateOrCreate(['id' => $data['id']], $season);
        } else {
            $dp = new DailyPrice;
            $dp->room_id = $data['room_id'];
            $dp->start_date = $data['start_date'];
            $dp->end_date = $data['end_date'];
            $dp->status = 'New';
            $dp->price = (int)$data['price'];
            $dp->save();
        }
	...
	public function delete_reservation(Request $request){
        ...
        $season_name = $request->season_name == null?"": $request->season_name ;
        $reservation = Calendar::where([
                    ['room_id',$request->id],
                    ['seasonal_name',$season_name],
                    ['start_date', $request->start_date],
                    ['end_date', $request->end_date]                                        
                    ])->where(function($query) {
                        $query->where([['status', 'Reserved'], ['source', 'Calendar']])->orWhere([['status', 'Reserved'], ['source', 'Reservation']])->orWhere('source', 'Import');
                    })->first();
        ...
        if($request->reservation_id!='' && $request->reserveid>0) {
            $reservationModel = Reservation::find($request->reservation_id);
            $reservationModel -> status = 'Cancelled';
            $reservationModel -> save();
        }
            ...
            $reservationModel = Reservation::find($request->reservation_id);
            if($reservationModel) $reservationModel->delete();
        }
	 ...
	 * @return bool
	 */
	public function check_reservation_conflict($room_id, $reservation_id, $checkin, $checkout, $edit_seasonal_name) {
//         
    ...
    public function check_seasonal_conflict($season) {
        ...
        $check_cal = SeasonalPrice::where([['room_id', $season->room_id], ['seasonal_name','!=',$season->edit_seasonal_name], ['start_date', '<=', $start_date], ['end_date', '>=', $start_date]])->orWhere([['room_id', $season->room_id], ['seasonal_name','!=',$season->edit_seasonal_name], ['start_date', '<=', $end_date], ['end_date', '>=', $end_date]])->get()->toArray();
	...
	public function save_seasonal_price(Request $request) {
        ...
        foreach($days as $day){
            ...
            $blocked_date = Calendar::where([
                ['room_id',$data->room_id],
                ['date',$day],
                ['status', 'Blocked']
            ])->first();
            if($blocked_date) continue; // if "Not Available" date exist, skip
    ...
    public function check_unavailable_conflict($data) {
        $room_id = $data['room_id'];
	...
	public function save_unavailable_dates(Request $request) {
        ...
        $checkout =  date('Y-m-d', strtotime($data->end_date));
        ...
        if($data->edit_seasonal_name !== ""){          // when edit unavailable dates
            ...
            // delete all records of the unavailable date range which is to be updated
            Calendar::where('room_id',$data->room_id)->where('seasonal_name',$data->edit_seasonal_name)->where('status', 'Blocked')->delete();
            // must recover season only after delete all records of the unavailable date range (because recovery function use updateOrCreate function)
        ...
        if($reservation_id>0) {
            $reservation = [     
                ...
                'currency_code' => 'USD',
                'ba_status' => 'Black',
                'title' => $data->seasonal_name,
            ];
            Reservation::updateOrCreate(['id' => $reservation_id], $reservation);    
        }else {
            $reservationModel = new Reservation;
            $reservationModel->room_id = $request->id;
            $reservationModel->checkin = $checkin;
            $reservationModel->checkout = $checkout;
            $reservationModel->reserveid = $data->reserveid;
            $reservationModel->status = 'New';
            $reservationModel->currency_code = 'USD';
            $reservationModel->ba_status = 'Black';
            $reservationModel->title = $data->seasonal_name;
            $reservationModel->save();
        }
	...
	public function save_reservation(Request $request) {
        ...
        $days = $this->get_days(strtotime($checkin),strtotime($checkout));        
        if($request->reservation_id>0){          // in case of edit             
            // recovery relevant season            
        ...
        if($request->reservation_id>0){          // when update reservation, delete original reservation
            $calendar = Calendar::where('room_id',$request->id)->where('seasonal_name',$request->edit_seasonal_name)->where('status', $status)->where('source', $source)->delete();            
        }
        $reserveid = $request->reseverid;
        ...
        if($reservation_id>0) {
            ...
            $reservationModel = new Reservation;
            $reservationModel->room_id = $request->id;
            $reservationModel->nights = $request->price;
            $reservationModel->checkin = $checkin;
            $reservationModel->checkout = $checkout;
            $reservationModel->number_of_guests = $request['guests'];
            $reservationModel->reserveid = $reserveid;
            $reservationModel->status = 'New';
            $reservationModel->currency_code = 'USD';
            $reservationModel->ba_status = 'Confirmed';
            $reservationModel->title = $request->seasonal_name;
            $reservationModel->notes = $notes;
            $reservationModel->save();
        }
