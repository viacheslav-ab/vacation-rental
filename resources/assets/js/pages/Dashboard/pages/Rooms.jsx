            ...
            <div className="listing-criteria-header">
              <div className="hide-sm show-md show-lg">
                <div className="field-wrapper">
                  {list.has_subscription == true &&  <select name="State" placeholder="State" className="form-in "  data-room-id={list.id} value={list.status} onChange={(event) => this.handleChangeStatus(list.id, event)}>
              ...
              {/* { list.has_subscription == false  ? <a  className="list-noti btn btn-primary text-white" href={`/rooms/${list.id}/subscribe_property`} target='_blank'>Subscribe</a> : <span className="list-noti">Subscribed</span>  } */}
