...
class Calendar extends Model
{
	 ...
	 * @var array
	 */
	protected $fillable = ['room_id', 'reserveid', 'price', 'status', 'date','start_date', 'end_date', 'notes', 'source', 'spots_booked', 'is_shared','week','month','additional_guest','guests','weekend','minimum_stay','seasonal_name'];
	...
	{//	dd($query);
		...
		})->whereIn('calendar.date', $days)->where(function($query){
			$query->where('calendar.status', 'Not available')->orWhere('calendar.status', 'Reserved');
		})
		// ->whereOr('calendar.status'  ,'Reserved')
