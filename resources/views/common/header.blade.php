...
<div id="header" class="makent-header {{@$header_class}} new {{ (!isset($exception)) ? (Route::current()->uri() == '/' ? 'shift-with-hiw' : '') : '' }}">
  <header class="header--sm show-sm" aria-hidden="true" role="banner">
    <a href="{{ url('/') }}" aria-label="Homepage" data-prevent-default="" class="header-logo rj_logo_mob" style="background-image: url({{ URL::asset('images/logo.png') }}); background-size: 100px;"></a>
  ...
  <header class="regular-header clearfix hide-sm bttm_border" id="old-header-hide" role="banner">
    <a aria-label="Homepage" style="background-image: url({{ URL::asset('images/logos/logo.jpg') }}); background-size: 145px;" href="{{ url('/') }}" class="header-belo header-logo pull-left {{ (!isset($exception)) ? (Route::current()->uri() == '/' ? 'home-logo' : '') : '' }}" >
      <span class="screen-reader-only">
      Vacation Rentals From Owners And Property Managers
      </span>
    </a>
