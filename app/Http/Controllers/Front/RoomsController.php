...
class RoomsController extends Controller
{
	...
	public function get_room_seasonal_rate(Request $request){
		...
		$data['seasonal_price_detail'] = SeasonalPrice::where([['room_id',$request->room_id]])->groupBy('seasonal_name')
	...
	public function room_details( Request $request){
		...
		$rooms = Rooms::where('id', $request->room_id)->where('status', 'Listed')->count();
		
		//dump($rooms);exit;
		//if(Rooms::find($request->room_id)){
		if($rooms > 0){
			$rooms_address = $data['result']->rooms_address;
			if($request->address_url != $data['result']->address_url){
				$address_url = $data['result']->address_url;
				$room_id = $request->room_id;
			...
			if($data['result']->has_subscription){
				return view('listingdetail', compact('roomdetail', 'rooms_description', 'featured_image'));
			}
			$error_message = 'Your listing is not subscribed!';
			return view('error', compact('error_message'));
			return "This listing is not subscribed in our Vacation.Rentals";
		}
		else{
			// abort(404);
			$url = "/";
