...
class RoomsPrice extends Model
{
	...
	public function price($date)
    {
        ...
        $whereBooking = [['room_id', $this->attributes['room_id']], ['checkin', '<=', $date], ['checkout', '>', $date], ['ba_status', 'Confirmed'], ['status', '<>', 'Cancelled']]; 
        
        $result = Reservation::where($whereBooking);

        if($result->count()) 
        {
            return $result->first()->nights;
        }
