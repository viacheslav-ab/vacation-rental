...
class PaymentHelper
{
	...
	public function price_calculation($room_id, $checkin, $checkout, $guest_count, $guest_options=[], $special_offer_id = '', $change_reservation='', $reservation_id ='')
	{
		...
		$result['cleaning_tax'] = 0;
		$result['additional_tax'] = 0;
			...
			foreach (json_decode($result['additional_charge']) as $key => $charge) {				
				...
				if($charge->guest_opt == 0) {	// yes
					...
					if(isset($guest_options[$charge->label]) && $guest_options[$charge->label] == 1) {
						...
						if ($charge->taxable == 'Yes') {
							$result['additional_tax'] += $add_charge_amount;
						}
		...
		//* blusky fix */
		if ($rooms_price->cleaning_taxable == 'Yes') {
			$result['cleaning_tax'] = $result['cleaning_fee'];
		}
		else
		{
			$result['cleaning_tax'] = 0;
		...
		$result['total_taxes_pay']    = (($result['night_price'] + $result['cleaning_tax'] + $result['additional_tax']) * $result['taxes_pay']) / 100;
